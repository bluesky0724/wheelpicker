...
import LetterWheel from '../Wheel/LetterWheel';
import Modal from 'react-modal';
import Select from 'react-select';
import { Nav } from './Nav';
import { LetterBoard } from '../Wheel/LetterBoard';
...
function Letter() {
  ...
  const [styleoption, setStyleOption] = useState('A');
  const [mainOption, setMainOption] = useState('A');

  const handleStyleOption = (item) => {
    setStyleOption(item.value);
  }


  const letters = [
    {content: 'A', type: 'Text', consonant : false},
    {content: 'B', type: 'Text', consonant : true},
    {content: 'C', type: 'Text', consonant : true},
    {content: 'D', type: 'Text', consonant : true},
    {content: 'E', type: 'Text', consonant : false},
    {content: 'F', type: 'Text', consonant : true},
    {content: 'G', type: 'Text', consonant : true},
    {content: 'H', type: 'Text', consonant : true},
    {content: 'I', type: 'Text', consonant : false},
    {content: 'J', type: 'Text', consonant : true},
    {content: 'K', type: 'Text', consonant : true},
    {content: 'L', type: 'Text', consonant : true},
    {content: 'M', type: 'Text', consonant : true},
    {content: 'N', type: 'Text', consonant : true},
    {content: 'O', type: 'Text', consonant : false},
    {content: 'P', type: 'Text', consonant : true},
    {content: 'Q', type: 'Text', consonant : true},
    {content: 'R', type: 'Text', consonant : true},
    {content: 'S', type: 'Text', consonant : true},
    {content: 'T', type: 'Text', consonant : true},
    {content: 'U', type: 'Text', consonant : true},
    {content: 'V', type: 'Text', consonant : true},
    {content: 'W', type: 'Text', consonant : false},
    {content: 'X', type: 'Text', consonant : true},
    {content: 'Y', type: 'Text', consonant : true},
    {content: 'Z', type: 'Text', consonant : true},
  ]



  const selectoptions = [
    { value: 'A', label: 'A to Z / a to z' , content: letters, filename: ''},
    { value: 'B', label: 'Consonant' , content: [], filename: ''},
    { value: 'C', label: 'Vowel' , content: [], filename: ''},
    { value: 'D', label: 'A to Z + a to z' , content: [], filename: ''},
    { value: 'E', label: 'Custom Letters' , content: [], filename: ''},
  ];

  const Styleselectoptions = [
    { value: 'A', label: 'UPPERCASE' , content: [], filename: ''},
    { value: 'B', label: 'lowercase' , content: [], filename: ''},
    { value: 'C', label: 'UPPERCASE & lowercase' , content: [], filename: ''},
    { value: 'D', label: 'Original' , content: [], filename: ''},
  ];
  ...
  const handleMenuOne1 = () => {
    const newImages = [];
    images.sort((a, b) => 0.5 - Math.random()).map((item)=>{
      newImages.push(item);
    });
    setImages(newImages);
  };
  ...
  const selectHandleChange = (selectedOption) => {
    setMainOption(selectedOption.value);
  ...
  useEffect(()=>{
    console.log(mainOption, styleoption);
    if(mainOption === 'A'){
      if(styleoption === 'A'){
        setImages(letters);
      }else if(styleoption === 'B'){
        setImages(letters.map((item)=> {
          item.content = item.content.toLowerCase();
          return item;
        }));
      }else if(styleoption === 'C'){
        setImages(letters.map((item)=> {
          item.content = item.content + item.content.toLowerCase();
          return item;
        }));
      }
    }else if(mainOption === 'B'){
      const consonants = letters.filter((item) => item.consonant === true);
      if(styleoption === 'A'){
        setImages(consonants);
      }else if(styleoption === 'B'){
        setImages(consonants.map((item)=> {
          item.content = item.content.toLowerCase();
          return item;
        }));
      }else if(styleoption === 'C'){
    ...
    }else if(mainOption === 'C'){
      const consonants = letters.filter((item) => item.consonant === false);
      if(styleoption === 'A'){
        setImages(consonants);
      }else if(styleoption === 'B'){
        setImages(consonants.map((item)=> {
      ...
      setImages(allletters);
    }else{
      const generatedLetters = nameSaveFile.split('');
      const newArray = [];
      ...
      if(styleoption === 'A'){
        ...
        setImages(newArray.map((item)=> {
          item.content = item.content.toUpperCase() + item.content.toLowerCase();
          return item;
        }));
      }else{
        setImages(newArray);
  ...
  },[styleoption, mainOption,nameSaveFile]);
  ...
  <LetterWheel items={images} setChoosenItem={setChoosenItem} setSavedItem={setSavedItem} setCountArr={setCountArr} countArr={countArr}/>
...
<div className="StyledBox-sc-13pk1d4-0 iOLvMP InputBox__StyledInputBox-sc-93rbkl-0 dqFYEQ" id="UploadImageArea" style={{display:'inline-flex !important',width:"500px"}}>
        ...
        <div className="StyledBox-sc-13pk1d4-0 cvKuOz results__Container-sc-1qwspq2-0 cBNarm mycss6">
          <div className='form-label'>Select Options :</div>
          <Select options={selectoptions} className="form-select" defaultValue={selectoptions[0]} onChange={selectHandleChange}/>
          <br/>
          {(mainOption === 'E') ? (
            <input type="text" className='form-input1' onChange={nameHandleChange} value={nameSaveFile}/>
          ) : ( <></> )}
          <br/>
          <div className='form-label'>Style Options :</div>
          {(mainOption === 'A' || mainOption === 'B' || mainOption === 'C') ? (
            <Select options={Styleselectoptions.slice(0,3)} className="form-select" defaultValue={Styleselectoptions[0]} onChange={handleStyleOption}/>
           ) : (mainOption === 'D') ? (
            <Select options={Styleselectoptions.slice(3)} className="form-select" defaultValue={Styleselectoptions[0]} onChange={handleStyleOption}/>
           ) : (
            <Select options={Styleselectoptions} className="form-select" defaultValue={Styleselectoptions[0]} onChange={handleStyleOption}/>
           )
          }
          <div className="StyledBox-sc-13pk1d4-0 fCkRfC ReactYNW__Container-sc-1wdn6wd-0 ktJwxo" style={{marginTop:"40px", width: "90%"}}>
            ...
            <div className="StyledBox-sc-13pk1d4-0 jxBtFZ">
              ...
              <div className="StyledBox__StyledBoxGap-sc-13pk1d4-1 epPYSv"></div>
              <button className="StyledButton-sc-323bzc-0 iiGFSH TooltipButton__Button-sc-rmqt3w-0 hITtYK" type="button" onClick={handleMenuOne1}>
...
{/* <div className="StyledBox-sc-13pk1d4-0 hCJgDx Title__TitleBox-sc-1unxr3s-0 dPLYnr">
  <div className="StyledTextInput__StyledTextInputContainer-sc-1x30a0s-1 imHyBg">
    ...
    <LetterBoard images={images} setImages={setImages} choosenItem={choosenItem} setChoosenItem={setChoosenItem} savedItem={savedItem} setSavedItem={setSavedItem}/>
